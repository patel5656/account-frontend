...
import { Share2, MessageCircle, Upload, Download } from 'lucide-react';
...
export function GstrPurchaseSummary() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [invoiceRes, customerRes] = await Promise.all([
        apiClient.get('/invoices?type=PURCHASE'),
      ...
      console.error('Error fetching GSTR Purchase Summary data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    link.download = `gstr_purchase_summary_${showHsnWise ? 'hsn' : 'standard'}.csv`;
    link.click();
  };
  ...
  const handleDownload = () => {
    ...
    // Title & Subtitle
    doc.setFontSize(16);
    doc.text('GST Purchase Summary', 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`gstr_purchase_summary_${showHsnWise ? 'hsn' : 'standard'}.pdf`);
  };
    ...
    <div className="bg-[#f4f6f9] min-h-[calc(100vh-45px)] p-4 flex flex-col relative pb-[80px]">
      ...
      <div className="bg-white rounded shadow-sm border border-gray-200 w-full overflow-hidden flex-1">
        ...
        <div className="text-center py-4 border-b border-gray-200">
          <h2 className="text-[14px] text-gray-700 mb-1">GST Purchase Summary</h2>
