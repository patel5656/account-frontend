...
import { getCategorywisePurchase } from '../api/financial';
...
export function CategorywisePurchaseSummary() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch categorywise purchase", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = () => {
    ...
    doc.setFontSize(18);
    doc.setTextColor(40);
    doc.text('Categorywise Purchase Summary', 14, 22);
    ...
    // Save the PDF
    doc.save(`categorywise_purchase_summary_${new Date().getTime()}.pdf`);
  };
