               ...
               <div className="relative w-full" ref={dropdownRef}>
                 ...
                 {isDropdownOpen && (
                   <div className="absolute top-full left-0 w-full mt-0.5 bg-white border border-gray-300 rounded-[3px] shadow-xl z-50 max-h-[300px] overflow-y-auto">
                     {incomes.filter(inc => inc.name.toLowerCase().includes(incomeSearch.toLowerCase())).map((inc) => (
                       <div 
                         key={inc.id} 
                         onClick={() => handleSelectIncome(inc)}
                         className={`p-2 border-b border-gray-200 hover:bg-[#add8e6] cursor-pointer flex justify-between ${selectedIncome?.id === inc.id ? 'bg-[#add8e6]' : 'bg-white'}`}
                       >
                         ...
                         <div className="flex flex-col items-end justify-between">
                           ...
                           <div className="flex gap-2 mt-1">
                             <Edit2 className="w-3.5 h-3.5 text-[#17a2b8] hover:text-cyan-700" onClick={(e) => handleEditIncome(e, inc.name)} />
                             <Trash2 className="w-3.5 h-3.5 text-[#dc3545] hover:text-red-700" onClick={(e) => handleDeleteIncome(e, inc.id)} />
                           </div>
                         </div>
                       </div>
                     ))}
                     {incomes.filter(inc => inc.name.toLowerCase().includes(incomeSearch.toLowerCase())).length === 0 && (
