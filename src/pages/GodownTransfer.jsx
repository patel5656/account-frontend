...
function GodownTransferForm({ onCancel, onSave }) {
  ...
  useEffect(() => {
    ...
    apiClient.get("/products?limit=100").then((res) => { if (res.data.data) setProducts(res.data.data); });
  }, []);
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            ...
            {(searchFocused || productName) && (
              products
                ...
                .map(p => (
                  <div key={p.id} className="grid grid-cols-[1fr_90px_140px] px-5 py-3 border-b border-gray-100 hover:bg-blue-50/40 cursor-pointer transition-colors">
                    <div>
                      <div className="text-[13px] font-semibold text-gray-800">{p.name}</div>
                      <div className="text-[11px] text-gray-400 mt-0.5">SKU: {p.sku} · <span className="text-blue-500 font-semibold">₹{p.sale}/Unit</span></div>
                    </div>
                      ...
                      <button
                        onMouseDown={(e) => {
                          ...
                          if (alreadyAdded) {
                            ...
                            setItems(prev => [...prev, { id: Date.now(), name: p.name, stock: p.stock, qty: '1', unit: p.baseUnit || 'PCS' }]);
                          }
                          setProductName('');
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-gray-400 mt-0.5">
                      {item.unit && <span className="text-blue-500 font-semibold">{item.unit}</span>}
                    </div>
                  </div>
                </div>
          ...
          <div className="px-5 py-3 border-t border-gray-200 grid grid-cols-3 bg-white">
            ...
            <div className="text-right">
              ...
              <div className="text-[15px] font-bold text-blue-600 mt-0.5">
                ₹{items.reduce((a, i) => {
                  const p = products.find(pr => pr.name.toLowerCase() === i.name.toLowerCase());
                  return a + (parseInt(i.qty || 0) * (p?.sale || 0));
                }, 0)}
              </div>
            </div>
          </div>
