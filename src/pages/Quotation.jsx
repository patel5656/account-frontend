...
export function Quotation() {
  ...
  const fetchInvoices = async () => {
    try {
      const res = await apiClient.get('/inventory/QUOTATION');
      if (res.data.data) {
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#f8f9fa] sticky top-0">
              <tr>
                ...
                <th className="px-4 py-3 text-[13px] font-bold text-gray-700 border-b border-gray-200">Quotation No</th>
            ...
            <tbody>
              ...
              {invoices.length === 0 && (
                <tr>
                  <td colSpan="6" className="text-center py-8 text-gray-500 text-[14px]">No Quotation records found.</td>
                </tr>
              )}
            </tbody>
          </table>
