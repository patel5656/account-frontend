...
export function EmployeewiseSaleSummary() {
  ...
  const handleExport = () => {
    const csvContent = [
      ['#', 'Employee Name', 'Total Quantity', 'Total Sale', 'Total Collection'],
      ['', 'Totals :', '0', '0', '0']
    ].map(e => e.join(",")).join("\n");
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "employeewise_sale_summary.csv";
    link.click();
  };
            ...
            <div className="flex flex-col sm:flex-row flex-1 gap-4 sm:gap-6">
              ...
              <div className="flex flex-col gap-1 w-full sm:max-w-[400px]">
                <div className="flex flex-wrap items-center gap-2 px-1">
                  ...
                  <label className="text-[13px] font-bold text-gray-800">Brand Name</label>
                </div>
                <select className="w-full h-[32px] border border-gray-300 rounded-[3px] px-2 text-[13px] outline-none text-gray-400 bg-white" disabled>
                  <option>Enter Brand Name</option>
                </select>
              </div>
            </div>

            {/* Empty right side to match screenshot layout where Discount Type is absent */}
