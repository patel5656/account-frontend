...
export function GstWiseSummary() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('Error fetching GST-wise data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  filteredInvoices.forEach(invoice => {
    ...
    invoice.items?.forEach(item => {
      ...
      gstSlabs[slabKey][groupKey].taxable += taxable;
      gstSlabs[slabKey][groupKey].tax += tax;
      gstSlabs[slabKey][groupKey].total += total;
    });
  });

  // Sort by rate ascending
  const displayRows = Object.values(gstSlabs).sort((a, b) => a.rate - b.rate);
  ...
  const handleExportCSV = () => {
    const headers = ['GST SLAB', 'Sales Taxable Amount', 'Sales Tax', 'Sales Total', 'Purchase Taxable Amount', 'Purchase Tax', 'Purchase Total'];
    let rowsContent = [];
    ...
    link.download = `gst_wise_summary.csv`;
    link.click();
  };
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: 'GST-wise Summary',
        text: 'Check out the GST-wise Summary',
    ...
    <div className="bg-[#f4f6f9] min-h-[calc(100vh-45px)] p-4 flex flex-col relative pb-[80px]">
      ...
      <div className="bg-white rounded shadow-sm border border-gray-200 w-full overflow-hidden flex-1">
        ...
        <div className="text-center py-4 border-b border-gray-200">
          <h2 className="text-[14px] text-gray-700 mb-1">GST-wise Summary</h2>
          ...
          <div className="table-scroll w-full overflow-x-auto">
            {loading ? (
              ...
              <table className="w-full border-collapse border border-black text-center">
                <thead>
                  <tr>
                    <th className="py-2.5 px-3 border border-black text-[13px] font-bold text-gray-800 w-[20%] whitespace-nowrap" rowSpan="2">GST SLAB</th>
                  ...
                  <tr>
                    <th className="py-2 px-3 border border-black text-[13px] font-bold text-[#dc3545] w-[15%] whitespace-nowrap">Taxable Amount</th>
                ...
                <tbody>
                  {displayRows.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="py-4 text-center text-[13px] text-gray-500">
                        No transactions found for the selected period.
                      </td>
                    </tr>
                  ) : (
                    displayRows.map(row => (
                      <tr key={row.slab} className="hover:bg-gray-50 transition-colors">
                        <td className="py-2.5 px-3 border border-black text-[13px] font-bold text-gray-800 text-left">{row.slab}</td>
